...
class GaussianAgent(BaseAgent): # policy를 Agent로 변경해야함
    ...
    def __init__(self,
        ...
        if not self.state_dependent_std:
            if learn_std:
                self.log_std = nn.Parameter(torch.full((self.policy.output_dim,), init_log_std))
            else:
                p = nn.Parameter(torch.full((self.policy.output_dim,), init_log_std), requires_grad=False)
    ...
    def distribution(self, obs: torch.Tensor) -> Independent:
        ...
        """
        device, dtype = module_device_dtype(self.policy) 
